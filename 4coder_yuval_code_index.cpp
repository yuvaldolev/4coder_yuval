...
index_new_note(Code_Index_File *index, Generic_Parse_State *state, Range_i64 range, Code_Index_Note_Kind kind, Code_Index_Nest *parent){
    ...
    Data data = make_data(result->text.str, result->text.size);
    Table_Lookup lookup = table_lookup(&index->string_to_index_note, data);
    if (!lookup.found_match){
        table_insert(&index->string_to_index_note, data, (u64)PtrAsInt(result));
    }
